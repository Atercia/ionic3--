...
export class HttpserviceService {
  ...
  testUrl = 'http://localhost:80/test/httpservice/testhttp.php';
  addAnswerSheetUrl = 'http://localhost:80/test/httpservice/addanswer.php';
...
addData(){
  ...
  .set('userid', 'test2')
  .set('title','test3');
  return this.http.post<any>(this.windowsLocalHostUrl,data,httpOptions).pipe(
...
//获取答卷  根据用户id
  ...
  let url = 'http://localhost:80/test/httpservice/PDphp.php';
  const data = new HttpParams()
  .set('userid', id)
...
adduser(id,name,tel,email,pw){
  let url = 'http://localhost:80/test/httpservice/reserve.php';
  const data = new HttpParams()
  .set('userid', id)
  .set('name',name)
  .set('tel', tel)
